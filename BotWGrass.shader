Shader "Custom/BotWGrass"
{
	Properties
	{
		...
		_PlayerCoords("Player Coords", Vector) = (0, 0, 0)
		_RenderDistance("Render Distance", Range(1, 1000)) = 20
	}
	...
	SubShader
	{
		...
		HLSLINCLUDE
			...
			CBUFFER_START(UnityPerMaterial)
				...
				bool _EnableRenderDistance;
				float3 _PlayerCoords;
			...
			float tessellationEdgeFactor_distanceTest(VertexInput vert0, VertexInput vert1)
			{
				...
				float viewDist = distance(edgeCenter, _PlayerCoords) / 10.0f;

				return edgeLength / (_TessellationGrassDistance * viewDist); //_ScreenParams.y
			}
			//float tessellationEdgeFactor_distanceTest(VertexInput vert0, VertexInput vert1)
			//{
			//	float3 v0 = vert0.vertex.xyz;
			...
			//	float3 edgeCenter = (v0 + v1) * 0.5f;
			//	float3 dist = pow(abs(_PlayerCoords.x - edgeCenter.x), 2) + pow(abs(_PlayerCoords.z - edgeCenter.z), 2);
			//	float viewDist = lerp(0, pow(_RenderDistance, 2), dist);

			// return edgeLength / (_TessellationGrassDistance + (2 - _TessellationGrassDistance) * viewDist);
			//}
			...
			float lineDist(float3 A,float3 B, float3 P)
			{
				...
				return sqrt((pow(av.x, 2) + pow(av.y, 2) + pow(av.z, 2))); // Use distance to start of segment instead.

				float3 bv = P - B;

				if (dot(bv, ab) >= 0.0)           // Point is advanced past the end of the segment, so perpendicular distance is not viable.
				 return sqrt((pow(bv.x, 2) + pow(bv.y, 2) + pow(bv.z, 2))); // Use distance to end of the segment instead.
	
				float3 tmp = cross(ab, av);

				return sqrt((pow(tmp.x, 2) + pow(tmp.y, 2) + pow(tmp.z, 2))) / sqrt((pow(ab.x, 2) + pow(ab.y, 2) + pow(ab.z, 2)));
			}
			...
			{
				float3 worldPos = mul(unity_ObjectToWorld, input[0].vertex).xyz;
				float grassVisibility = tex2Dlod(_GrassMap, float4(input[0].uv, 0, 0)).r;
	
				

				if (grassVisibility >= _GrassThreshold && pow(abs(_PlayerCoords.x - worldPos.x), 2) + pow(abs(_PlayerCoords.z - worldPos.z), 2) < pow(_RenderDistance, 2))
				{
					bool tmp = true;
					for (int i = 0; i < 31; i++)
					{
						if (lineDist(float3(_path[i].x, 0, _path[i].z), float3(_path[i + 1].x, 0, _path[i + 1].z), float3(worldPos.x, 0, worldPos.z)) < 2)
						{
							tmp = false;
						}
					...
					if(tmp)
					{
						//float3 pos = input[0].vertex.xyz; //ADD RANDOMNESS HERE ------------------------------------------------

						float3 pos = input[0].vertex.xyz + float3(0.5, 0.0, 0.5) * rand(input[0].vertex.xyz);
						...
						float3x3 windMatrix = angleAxis3x3((float) (UNITY_PI * windSample), windAxis);
						...
						for (int i = 0; i < BLADE_SEGMENTS; ++i)
						{
							...
							triStream.Append(TransformGeomToClip(pos, float3(offset.x, offset.y, offset.z), transformationMatrix, float2(0, t), input[0].normal));
							triStream.Append(TransformGeomToClip(pos, float3(-offset.x, offset.y, offset.z), transformationMatrix, float2(1, t), input[0].normal));
						}

											// Add the final vertex at the tip of the grass blade.
						triStream.Append(TransformGeomToClip(pos, float3(0, forward, height), tipTransformationMatrix, float2(0.5, 1), input[0].normal));
        ...
        Pass
        {
            ...
            float4 frag (GeomData i) : SV_Target
            {
				    ...
				    surfaceData.albedo = tex2D(_BladeTexture, i.uv) * lerp(_BaseColor, _TipColor, i.uv.y).rgb;
					...
					surfaceData.alpha = tex2D(_BladeTexture, i.uv).w;
					surfaceData.clearCoatMask = 0;
					surfaceData.clearCoatSmoothness = 1;
	
					InputData lightningInput = (InputData) 0;
	
					lightningInput.positionWS = i.pos;
					lightningInput.normalWS = normalize(i.normal);
					lightningInput.viewDirectionWS = GetWorldSpaceNormalizeViewDir(i.pos);
					lightningInput.shadowCoord = TransformWorldToShadowCoord(i.pos);
